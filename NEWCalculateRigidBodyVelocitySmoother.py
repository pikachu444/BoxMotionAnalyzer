...
import argparse import numpy as np import pandas as pd from scipy.spatial.transform import Rotation from scipy.interpolate import UnivariateSpline # For spline fitting from scipy.signal import butter, filtfilt # For Butterworth filter

--- Configuration Loading ---
import config BOX_DIMS = config.BOX_DIMS LOCAL_BOX_CORNERS = config.LOCAL_BOX_CORNERS

--- Utility / Filter Functions ---
def apply_butter_lowpass_filter_to_series(data_series_1d, cutoff_hz, fs, order): """Applies a Butterworth low-pass filter to a 1D data series.""" if cutoff_hz is None or cutoff_hz <= 0 or fs <= 0: return data_series_1d # No filtering if params invalid or fs not available nyquist_freq = 0.5 * fs if cutoff_hz >= nyquist_freq: print(f" Warning: Low-pass cutoff ({cutoff_hz}Hz) >= Nyquist ({nyquist_freq}Hz). Filtering skipped for series.") return data_series_1d
...
try:
    ...
    return data_series_1d
def apply_filter_to_ndim_data(data_ndim, time_s, fs, filter_func, **filter_params): """Applies a 1D filter_func to each column of N-D data.""" filtered_data = np.copy(data_ndim) for i_col in range(data_ndim.shape[1]): filtered_data[:, i_col] = filter_func(data_ndim[:, i_col], **filter_params, fs=fs, time_s=time_s) # Pass fs and time_s if needed by filter_func return filtered_data

def apply_moving_average_to_series(data_series_1d, window_size): """Applies moving average to a 1D data series.""" if window_size is None or int(window_size) <= 1: return data_series_1d # Pandas rolling mean handles NaNs by default (keeps them NaNs in output or skips them based on min_periods) return pd.Series(data_series_1d).rolling(window=int(window_size), center=True, min_periods=1).mean().values

def apply_ma_to_ndim_data(data_ndim, window_size): """Applies moving average to each column of N-D data.""" smoothed_data = np.copy(data_ndim) for i_col in range(data_ndim.shape[1]): smoothed_data[:, i_col] = apply_moving_average_to_series(data_ndim[:, i_col], window_size) return smoothed_data

def numerical_derivative_finite_diff(y_values, t_values): """Calculates numerical derivative using finite differences (handles varying dt).""" if len(y_values) != len(t_values): raise ValueError("y/t length mismatch.") if len(y_values) < 2: return np.zeros_like(y_values, dtype=float) y_np, t_np = np.asarray(y_values), t_values.astype(float) dy_dt = np.zeros_like(y_np, dtype=float) dt_0 = t_np[1] - t_np[0] dy_dt[0] = (y_np[1] - y_np[0]) / dt_0 if dt_0 > 1e-9 else 0.0 for i in range(1, len(y_np) - 1): den = t_np[i+1] - t_np[i-1] if abs(den) <= 1e-9: dt_f, dt_b = t_np[i+1]-t_np[i], t_np[i]-t_np[i-1] if dt_f > 1e-9 and dt_b > 1e-9: dy_dt[i] = 0.5*(((y_np[i+1]-y_np[i])/dt_f) + ((y_np[i]-y_np[i-1])/dt_b)) elif dt_f > 1e-9: dy_dt[i] = (y_np[i+1]-y_np[i])/dt_f elif dt_b > 1e-9: dy_dt[i] = (y_np[i]-y_np[i-1])/dt_b else: dy_dt[i] = 0.0 else: dy_dt[i] = (y_np[i+1]-y_np[i-1])/den if len(y_np) > 1: dt_N = t_np[-1] - t_np[-2] dy_dt[-1] = (y_np[-1]-y_np[-2])/dt_N if dt_N > 1e-9 else 0.0 return dy_dt

def ensure_quaternion_continuity(quaternions_scipy_fmt): """Ensures quaternion continuity (SciPy format [x,y,z,w]) by flipping signs.""" q_proc = np.copy(quaternions_scipy_fmt) for i in range(1, len(q_proc)): if np.dot(q_proc[i-1], q_proc[i]) < 0: q_proc[i] *= -1 return q_proc

--- Velocity Calculation Core Functions ---
def calculate_translational_velocity_spline(positions_t, time_s, s_factor=0, spline_degree=3): """Calculates translational velocity from positions using spline differentiation.""" num_frames, num_dims = positions_t.shape velocities_t = np.zeros_like(positions_t) print(f" Spline for Translation: s_factor={s_factor}, degree={spline_degree}") for dim_idx in range(num_dims): try: # UnivariateSpline requires sorted x (time_s). Ensure this. # If time_s is not strictly increasing, spline fitting will fail. # A check for sorted time_s should be done before calling this. if not np.all(np.diff(time_s) > 0): # Check if time is strictly increasing # Attempt to sort if not, but this implies data might be misaligned sort_indices = np.argsort(time_s) time_s_sorted = time_s[sort_indices] positions_t_sorted_dim = positions_t[sort_indices, dim_idx] if not np.all(np.diff(time_s_sorted) > 1e-9): # Check for duplicate times after sorting raise ValueError("Time values are not strictly increasing or contain duplicates, cannot fit spline.") else: time_s_sorted = time_s positions_t_sorted_dim = positions_t[:, dim_idx]
...
return velocities_t
def calculate_angular_velocity_spline_from_quaternions(quaternions_continuous_t, time_s, s_factor=0, spline_degree=3): """Calculates angular velocity (world) from quaternions using spline differentiation.""" num_frames, num_q_comps = quaternions_continuous_t.shape angular_velocities_world_t = np.zeros((num_frames, 3)) if num_frames < 2: return angular_velocities_world_t print(f" Spline for Rotation (Quaternions): s_factor={s_factor}, degree={spline_degree}")
...
def calculate_angular_velocity_matrix_based_fd(rot_vectors_rad, time_s): """FD-based angular velocity (world) using rotation matrix differentiation.""" # (Implementation from previous version, uses numerical_derivative_finite_diff) num_frames = len(rot_vectors_rad) angular_velocities_world = np.zeros((num_frames, 3)) if num_frames < 2: return angular_velocities_world rot_matrices = Rotation.from_rotvec(rot_vectors_rad).as_matrix() dR_dt = np.zeros_like(rot_matrices) for r_idx in range(3): for c_idx in range(3): dR_dt[:, r_idx, c_idx] = numerical_derivative_finite_diff(rot_matrices[:, r_idx, c_idx], time_s) for i in range(num_frames): R_i, dR_dt_i = rot_matrices[i], dR_dt[i] omega_skew_matrix = dR_dt_i @ R_i.T omega_skew_antisym = 0.5 * (omega_skew_matrix - omega_skew_matrix.T) angular_velocities_world[i] = [omega_skew_antisym[2,1], omega_skew_antisym[0,2], omega_skew_antisym[1,0]] return angular_velocities_world

def calculate_angular_velocity_quaternion_based_fd(rot_vectors_rad, time_s): """FD-based angular velocity (world) using quaternion differentiation.""" # (Implementation from previous version, uses numerical_derivative_finite_diff) num_frames = len(rot_vectors_rad) angular_velocities_world = np.zeros((num_frames, 3)) if num_frames < 2: return angular_velocities_world quaternions_scipy = Rotation.from_rotvec(rot_vectors_rad).as_quat() continuous_quaternions = ensure_quaternion_continuity(quaternions_scipy) dq_dt_scipy = np.zeros_like(continuous_quaternions) for k_comp in range(4): dq_dt_scipy[:, k_comp] = numerical_derivative_finite_diff(continuous_quaternions[:, k_comp], time_s) for i in range(num_frames): q_i, dq_dt_i = continuous_quaternions[i], dq_dt_scipy[i] w_s,x_s,y_s,z_s = q_i[3],q_i[0],q_i[1],q_i[2] dw_s,dx_s,dy_s,dz_s = dq_dt_i[3],dq_dt_i[0],dq_dt_i[1],dq_dt_i[2] omega_body_x = 2*(-x_sdw_s + w_sdx_s + z_sdy_s - y_sdz_s) omega_body_y = 2*(-y_sdw_s - z_sdx_s + w_sdy_s + x_sdz_s) omega_body_z = 2*(-z_sdw_s + y_sdx_s - x_sdy_s + w_sdz_s) ang_vel_body = np.array([omega_body_x, omega_body_y, omega_body_z]) R_i = Rotation.from_quat(q_i).as_matrix() angular_velocities_world[i] = R_i @ ang_vel_body return angular_velocities_world

--- Main Processing Pipeline Function ---
def process_kinematics_pipeline(df_input, velocity_method_flag, use_pose_lpf_flag, pose_lpf_cutoff_val, pose_lpf_order_val, use_pose_ma_flag, pose_ma_window_val, spline_s_pos_val, spline_s_rot_val, spline_k_val, fd_angular_method_val, use_vel_lpf_flag, vel_lpf_cutoff_val, vel_lpf_order_val): """ Main pipeline to process pose data and calculate velocities with optional smoothing/filtering. """ df = df_input.copy() time_s = df['Time'].values.astype(float)
...
return df
def main(): # ============================================================================== # USER-CONFIGURABLE PIPELINE STAGES & PARAMETERS # ============================================================================== # Set 'active' to True to enable a stage, False to disable. # Parameters are used only if the stage is active.
...
VELOCITY_CALCULATION_METHOD = 'spline'#'finite_difference' #'spline'
