...
- 변경은 사용자에게 실제 기능을 제공하는 의미 있는 단위로 나눈다.
- 승인된 작업 범위에서는 구현, 필요한 실행 확인과 독립 리뷰, 지적 수정, 커밋과 push를 별도 승인 요청 없이 진행한다. 이슈 완료 조건을 충족하면 PR을 만들거나 갱신하고 필수 CI 통과 후 병합한다.
- 사용자가 정해야만 하는 시험 조건, 요구사항 또는 권한이 빠져 진행할 수 없을 때만 질문하고 답을 기다린다. 이미 승인한 내용을 다시 묻지 않는다.
- 현재 작업을 마친 뒤 다음 작업으로 넘어간다. 의미 없는 형식이나 부수적인 예외 처리에 머무르지 말고 실험 데이터 분석과 사용자가 활용할 결과를 우선한다.
- UI에는 필요한 조작과 결과를 간결하게 표시한다. 설명 문구를 늘어놓거나 `•` 기호로 제목과 단어를 길게 조합하지 않는다.
