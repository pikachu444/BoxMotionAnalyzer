...
자세 시간곡선, 기준 파일과의 차이, 접촉 빈도·기준 범주 일치는 이미 구현되어 있다. 이 기능들은 재사용하며, 남은 소프트웨어 범위는 알려진 시험 의도와 관측 첫 접촉의 일치 분석이다. #77은 이 기능 때문에 계속 열린다. 실측 접촉 시점·미분·방향·비교 정확도 검증은 #78에서 pending으로 추적한다.
