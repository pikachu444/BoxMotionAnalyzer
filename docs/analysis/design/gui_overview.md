...
PR #109의 CI와 실제 화면 대조에서 기본 배율의 요약 여백 부족, Contact의 두 줄 행 높이가 다른 보기에도 남는 현상, 잘린 3D 표본 표시, 보기 전환 후 한 글자로 줄어드는 범례를 확인했다. 요약은 현재 내용으로 행 높이를 재계산하고, 3D 안에서 표본 표시 공간을 확보하며, 범례는 읽을 수 있는 폭을 유지해 가로로 스크롤한다. 화면·그래프 최소높이·폰트·원본 수치·물리 허용오차는 유지했다. 같은 저장 파일을 사용한 작은 창의 첫·마지막 행, 표본·시간, 파일명 범례와 8코너 렌더를 재확인하며 필수 CI는 최종 head에서 별도로 판정한다.
