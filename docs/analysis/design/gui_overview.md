...
마커 보정 검토 창은 왼쪽에 원본 관측 XYZ 그래프, 오른쪽에 기존 로컬 축 미리보기를 함께 표시한다. 마커/강체 중심 선택, 확대·이동, `Around event`를 사용할 수 있다. 사건을 바꾸면 주변 시각으로 이동하지만 축·Apply 변경은 조정한 그래프 범위와 다른 사건의 선택을 유지한다. 원본 누락은 공백으로 남으며 그래프 탐색이 승인이나 보정 저장을 수행하지 않는다. [검토 창 배치와 검증](../../visualization/marker_review_context_layout.md)을 참고한다.
...
한 면의 마커 3개가 일직선이라는 이유만으로 JSON 배치를 거부하지 않는다. 전체 관측으로 자세를 계산할 수 있는지는 분석 단계에서 확인한다. 새 UI나 배치 인증 기능은 추가하지 않았다. [배치·접촉 조사](../reference/layout_and_contact_investigation.md)에 실제 복구와 #120 보류 근거를 기록했다.
...
`Detect scenes`는 고정 마커 배치의 상대 운동으로 낙하 후보, 기울임·회전, 취급 후보, 자세 점프, 대기·추적 불가 구간을 찾는다. 별도 수동 관리 팝업은 사용하지 않는다. 기존 그래프 아래 목록에서 구간을 선택하면 `Slice Range`와 그래프 선택 범위가 정확한 초 단위 값으로 동기화된다. `Signal`은 처음 검출할 때 `Vertical speed (mm/s)`를 선택하고 바로 다음에 상대 회전을 둔다. 같은 파일의 재검출은 현재 선택한 원본/파생 신호에 유한한 샘플이 있으면 그 선택을 유지한다. 신호가 사라지거나 모두 비어 있으면 수직 속도, 상대 회전, 원본 위치 순으로 유효한 신호를 선택한다. 미등록 수직 속도는 마커 배치 기준점의 월드 Y 속도이며 상자 중심·질량 중심 검증값이 아님을 툴팁으로 설명한다. 축에는 이름과 단위를 표시한다. 마커 수와 정합 잔차도 기존 선택 상자에서 볼 수 있다. 상대 회전은 최초 유효 자세와의 각도이며 누적 회전수는 아니다.
...
Step 1에는 파일 로드·구간 선택에 필요한 원본 위치와 상대 운동 신호, 검출 목록을 표시한다. 목록 아래의 모서리·높이·단계 요약과 `Least-moving edge travel (mm)`, `Opposite edge height (mm)` 선택지는 제거했다. 기존 작업에 두 신호가 저장돼 있으면 `Vertical speed (mm/s)` 등 유효한 기본 신호로 열며, 원본 Position-Y와 마커 선택은 그대로 복원한다. 높이와 고정 모서리 근거는 자동 구간 연결과 저장 이력에 계속 사용하며 별도로 입력하거나 알아야 하는 값이 아니다.
