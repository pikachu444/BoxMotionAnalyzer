...
새 `.slice`의 검토 이력은 Step 1.5의 처리와 `.proc` 재열기에도 전달된다. 파일 하나는 선택한 구간의 근거와 전체 포함·제외 결정 목록을 보존한다. 현재는 전체 촬영의 편집 작업을 복원하는 프로젝트 저장 기능, 시험 기록 기반 순서 추론, G16/G17 조건 및 H 지지·회전 시험의 완전한 항목 식별까지 제공하지 않는다.

박스·바닥을 등록한 경우 목록 위에 선택 구간의 고정 모서리 후보, 최소 모서리 이동량과 반대 모서리의 최대 높이를 표시한다. `Signal`의 `Least-moving edge travel (mm)`와 `Opposite edge height (mm)`는 선택한 범위 전체의 경로를 그린다. 중심이 제자리이거나 시작·끝 자세가 같다는 이유만으로 고정 지지라고 판단하지 않는다. 바닥 관통, 추적 공백, 여러 피벗 후보 또는 작은 회전은 보류하고, 바닥 위쪽에 고정된 모서리도 지지 장치를 확정하지 않는다.

이 측정의 시작은 관측 범위의 첫 자세다. 현재 검출기는 느린 최고점 자세를 사이에 둔 상승과 복귀를 별도 운동 후보로 남길 수 있다. 왕복 전체를 보고 싶으면 기존 `Slice Range`를 확장한 뒤 재검출한다. 선택 범위 전체가 고정된 바닥 모서리 회전과 양립하면 일반 운동을 기울임·회전으로 다시 분류하지만, 시험 횟수나 H 항목 번호를 확정하지 않는다. 측정 근거는 검토 JSON에 보존되며 범위·등록·치수가 바뀌면 무효화된다.
