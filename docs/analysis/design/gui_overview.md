...
PR #109의 첫 CI 34774596187은 기본 Windows 배율에서 Summary 첫 셀 29px가 본문 25px에 잘려 실패했다. 실제 Windows에서 같은 반례를 재현하고 Summary 내부 여백·간격만 줄였다. 화면·표 최소높이·폰트·기대값은 그대로이며, 동일 검사가 기본 배율과 125%에서 모두 통과했다. 필수 hosted 재실행은 수정된 head에서 별도로 판정한다.

두 번째 CI 34775000191은 Qt 6.11.2의 컬럼 선택에서 실패했다. 같은 컬럼 값도 서로 다른 tuple 객체이면 `QComboBox.findData`가 찾지 못하는 동작을 해당 버전으로 재현했다. 비교·Step 2의 선택 복원과 GUI 재생은 tuple/list의 값을 비교하도록 함께 수정했다. 해당 버전의 실제 Windows 실행에서 비교 화면과 Step 2의 선택 재구성·실패 복원·코너 선택점 내보내기가 통과했다. 컬럼 이름과 원본 데이터·기대값은 바꾸지 않았다.
