...
- [ ] Complete #77 intended first-contact agreement. Existing posture curves, baseline differences and contact summaries are already implemented and will be reused. The user confirmed these experiments have no target angle: nominal-angle error is outside scope, not a missing-reference state. Independently labeled real metrics and comparison validation remain pending in #78; the first unit does not complete the entire issue.
