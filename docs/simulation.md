...
1. 박스 크기(Width, Length, Height)와 질량(Mass) 입력.
2. 낙하 높이(Drop Height, mm) 및 낙하 자세 시나리오 선택.
3. [Run Simulation] 클릭 시 백그라운드에서 물리 연산 후 `.proc` 저장.
4. 이후 메인 [Analysis & Visualization] 탭에서 생성된 파일을 로드하여 3D 시각화 가능.
...
- **크기 (Dimensions):**
  - 사용자가 입력하는 전체 크기(가로, 세로, 높이)는 내부적으로 **절반 길이(Half-extents)** 로 변환되어 MuJoCo `geom` 속성에 적용됩니다.
- **무게 중심 (Center of Mass, CoM):**
  - 박스의 실제 내용물 배치에 따라 무게 중심이 다를 수 있습니다. 이를 반영하기 위해 `com_offset` 변수를 통해 질량 중심을 임의로 이동시킬 수 있습니다.
  - 예: TV 낙하 시나리오(`test_tv_drop.py`)의 경우, 디스플레이 패널의 무게로 인해 Z축이나 X/Y축 방향으로의 치우침을 모델링할 수 있습니다.
