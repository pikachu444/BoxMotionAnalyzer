...
GUI는 두 개의 주요 기능 영역으로 구성된 2분할 구조를 가집니다. 이는 사용자가 원본 데이터를 분석하는 작업과, 이미 분석된 결과를 시각화하는 작업을 명확히 분리하여 사용성을 개선합니다.

- **Original Data Analysis 영역:** 원본 CSV 파일을 로드하고, 분석 파라미터를 설정하며, 전체 분석 파이프라인을 실행하고, 그 결과를 내보내는 핵심 기능을 담당합니다.
- **Result Analyzer 영역:** 분석이 완료되어 저장된 결과 CSV 파일들을 불러와, 다양한 데이터 조합을 선택하여 시각화하고 비교/분석하는 보조 기능을 제공합니다.

이 새로운 구조는 `gui_sketch.txt`와 `gui_description.md`에 상세히 기술되어 있습니다.
...
*(상세 기능 및 책임은 `component_specs.txt` 파일 참조 - 향후 업데이트 필요)*
