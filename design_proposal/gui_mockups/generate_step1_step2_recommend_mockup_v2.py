...
class Step1MockWindowV2(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('Step 1 - Raw Data Processing (v2)')
...
class Step2MockWindowV2(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('Step 2 - Results Analysis (v2)')
        ...
        popup_state = QGroupBox('Popup Plot State')
        popup_state_l = QHBoxLayout(popup_state)
        popup_state_l.addWidget(QLabel('Opened Windows: 1'))
        popup_state_l.addSpacing(12)
        popup_state_l.addWidget(QLabel('Active: Velocity_XYZ'))
        popup_state_l.addStretch()
        popup_state_l.addWidget(QPushButton('Close All Popups'))
        ...
        # Main preview: single selected column only
        plot_group = QGroupBox('Main Plot Preview (Single Selected Column)')
        plot_l = QVBoxLayout(plot_group)
        canvas = FigureCanvas(Figure(figsize=(11, 3), dpi=100))
        ax = canvas.figure.subplots()
        x = np.linspace(20, 30, 300)
        y = 0.4 * np.sin(x * 2.2)
        ax.plot(x, y, color='#2b6cb0', label='Velocity/X')
        ax.set_xlabel('Time (s)')
        ax.set_ylabel('Value')
        ax.grid(True, alpha=0.3)
        ax.legend(loc='upper right')
...
class PopupMockWindowV2(QDialog):
    def __init__(self):
        super().__init__()
...
if __name__ == '__main__':
    ...
    step1 = Step1MockWindowV2()
    step2 = Step2MockWindowV2()
    popup = PopupMockWindowV2()

    save_widget(step1, os.path.join(out_dir, 'step1_improved_mock_v2.png'))
    save_widget(step2, os.path.join(out_dir, 'step2_improved_mock_v2.png'))
    save_widget(popup, os.path.join(out_dir, 'step2_popup_improved_mock_v2.png'))

    print('Saved:')
    print(os.path.join(out_dir, 'step1_improved_mock_v2.png'))
    print(os.path.join(out_dir, 'step2_improved_mock_v2.png'))
    print(os.path.join(out_dir, 'step2_popup_improved_mock_v2.png'))
