...
def slice_raw_data_csv(input_filepath, output_filepath, filter_by_type, start_value, end_value):
    """
    Slices a raw experimental data CSV file based on frame number or time range.
    The first 8 lines (header/metadata) are preserved. Data lines are filtered.
    ...
    try:
        with open(input_filepath, mode='r', encoding='utf-8-sig') as infile, \
            ...
            for i, line_content in enumerate(infile):
                ...
                # This logic mirrors AlignBoxInputGenbyExperiment.py's extraction
                    ...
                    if filter_by_type == 'frame':
                        if frame_number_str_raw:
                            try:
                                ...
                                # print(f"Warning: Could not parse frame number '{frame_number_str_raw}' on line {lines_read}. Skipping line.")
                                continue # Skip if frame number cannot be parsed
                        else: # Frame number string is empty
                            # print(f"Warning: Empty frame number on line {lines_read}. Skipping line.")
                            continue
                        ...
                        if time_value_str_raw:
                            try:
                                ...
                                # print(f"Warning: Could not parse time value '{time_value_str_raw}' on line {lines_read}. Skipping line.")
                                continue # Skip if time cannot be parsed
                        else: # Time string is empty
                            # print(f"Warning: Empty time value on line {lines_read}. Skipping line.")
                            continue
                
                except IndexError:
                    # print(f"Warning: Line {lines_read} too short to extract frame/time. Skipping line.")
                    continue # Skip malformed lines

                # Apply filter
                if current_value_for_filter is not None:
                    if start_val_typed <= current_value_for_filter <= end_val_typed:
                        outfile.write(line_content) # Write the original line content (with newline)
                        lines_written += 1
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Slices a raw experimental data CSV based on frame number or time range. "
                    "The first 8 lines (headers/metadata) are preserved."
    )
    parser.add_argument("input_raw_csv", type=str,
                        help="Path to the input raw experimental data CSV file.")
    parser.add_argument("output_sliced_csv", type=str,
                        help="Path to save the sliced (filtered) raw CSV file.")
    parser.add_argument("--filter_by", type=str, required=True, choices=['frame', 'time'],
                        help="Criterion for filtering: 'frame' or 'time'.")
    parser.add_argument("--start_val", type=str, required=True, # Read as string, convert later
                        help="Starting value for the filter range (inclusive). Integer for 'frame', float for 'time'.")
    parser.add_argument("--end_val", type=str, required=True,   # Read as string, convert later
                        help="Ending value for the filter range (inclusive). Integer for 'frame', float for 'time'.")

    args = parser.parse_args()

    # Convert start_val and end_val to appropriate types based on filter_by
    # This is now done inside the function for better error handling context.
    # try:
    #     if args.filter_by == 'frame':
    #         start_value_typed = int(args.start_val)
    #         end_value_typed = int(args.end_val)
    #     else: # 'time'
    #         start_value_typed = float(args.start_val)
    #         end_value_typed = float(args.end_val)
    # except ValueError:
    #     print(f"Error: Invalid start/end value for filter type '{args.filter_by}'. Please provide numbers.")
    #     exit(1)

    slice_raw_data_csv(args.input_raw_csv, 
                       args.output_sliced_csv, 
                       args.filter_by, 
                       args.start_val, # Pass as string, function will convert
                       args.end_val)   # Pass as string, function will convert
